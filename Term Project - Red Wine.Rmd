...
```{r}
cor.red = cor(red.wine[,-12])
corrplot.mixed(cor.red, lower.col = 'black', number.cex = .7)
```
<br />
...
hii = diag(H)
Hmax = max(diag(H))
plot(hii, main = "H_ii Values for Red Wine")
```
<br />
...
Identifying the points on the plot, 653 and 833rd data are potential outliers. 
...
max.di
plot(red.cooksd, pch="*", cex=2, ylab = "Cook's Distance", main="Influential Observations by Cook's Distance") 
...
```{r}
red.wine.del2 = red.wine[-c(152,653,1236), ]
...
```{r}
red.ols = ols_step_best_subset(red.mdl)
plot(red.ols)
best.red = lm(quality ~ volatile.acidity + chlorides + free.sulfur.dioxide + total.sulfur.dioxide + pH                + sulphates + alcohol, data=red.wine)
...
```{r}
start.mdl = lm(quality~1, data=red.wine)
scope.mdl = lm(quality~., data=red.wine)
...
```{r}
start.mdl2 = lm(quality~., data=red.wine)
...
```{r}
set.seed(71168)
sample.n = ceiling(0.8*length(red.wine$quality))

for(i in 1:5){
  train.sample = sample(c(1:length(red.wine$quality)),sample.n)
  train.sample = sort(train.sample)
  
  train.data = red.wine[train.sample,]
  test.data = red.wine[-train.sample,]
