...
identify(red.mdl$fitted.values, ti)
```
<br / >
...
red.sum = summary(red.mdl.del)
red.sum.del = summary(red.mdl)
red.mse = mean(red.sum$residuals^2)
red.mse.del = mean(red.sum.del$residuals^2)
red.sum$coefficients
red.sum.del$coefficients
red.mse.del
red.mse
```
<br />

Deleting the potential outlier points almost has no effect on the estimates of the regression coefficients nor on the residual mean square. In fact, deleting the points causes a decrease in $R^2$, which does not seem desirable. Hence, we conclude that points 653 and 833 are not influential.
...
red.log.mse = mean(red.log.sum$residuals^2)
red.sum
red.log.sum
((red.mse - red.log.mse)/red.mse)*100
```
<br />

By taking logarthims of quality values, estimates of each of the coefficients changed quite a lot. Also, the mean squared error for the regression line decreased by 96.59%, which is a lot. However, the log transformation decreased the $R^2$ value, which is not good.
...
red.sqr.mse = mean(red.sqr.sum$residuals^2)
red.sum
red.sqr.sum
red.mse
red.sqr.mse
((red.mse - red.sqr.mse)/red.mse)*100
```
<br />

By taking the square-root of quality values, estimates of each of the coefficients changed quite a lot. Also, the mean squared error for the regression line decreased by 95.39%, which is a lot. However, the square-root transformation decreased the $R^2$ value, which is not good.
