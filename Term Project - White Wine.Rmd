...
white.hist_x4 = ggplot(aes(residual.sugar), data = white.wine) +
  geom_histogram(aes(color=I('black'), fill=I('firebrick')), binwidth = 5) +
...
white.hist_x6 = ggplot(aes(free.sulfur.dioxide), data = white.wine) +
  geom_histogram(aes(color=I('black'), fill=I('firebrick')), binwidth = 10) +
...
white.hist_x7 = ggplot(aes(total.sulfur.dioxide), data = white.wine) +
  geom_histogram(aes(color=I('black'), fill=I('firebrick')), binwidth = 16) +
...
Let's investigate the pair-wise correlation among the regressors.
...
It is fairly easy to inadvertently extrapolate when doing multiple regression, so we will go through the formal procedure to look for hidden extrapolation. 

First, we will obtain the hat matrix to see if certain data points are within the regressor variable hull (RVH).
...
hii2 = diag(H2)
Hmax2 = max(diag(H2))
plot(hii2, ylab="hii", main = "h_ii Values for White Wine")
```
<br />
...
There are a couple of methods for dealing with multicollinearity.

One approach we can take for model respecification is to redefine the regressors. The advantage of such process is that it may lower the VIF values, while keeping the interpretation of coefficients intact.

```{r}
fixed.cen = (white.wine.del$fixed.acidity - mean(white.wine.del$fixed.acidity))
vol.cen = (white.wine.del$volatile.acidity - mean(white.wine.del$volatile.acidity))
citric.cen = (white.wine.del$citric.acid - mean(white.wine.del$citric.acid))
resid.cen = (white.wine.del$residual.sugar - mean(white.wine.del$residual.sugar))
chlor.cen = (white.wine.del$chlorides - mean(white.wine.del$chlorides))
free.cen = (white.wine.del$free.sulfur.dioxide - mean(white.wine.del$free.sulfur.dioxide))
tot.cen = (white.wine.del$total.sulfur.dioxide - mean(white.wine.del$total.sulfur.dioxide))
dens.cen = (white.wine.del$density - mean(white.wine.del$density))
ph.cen = (white.wine.del$pH - mean(white.wine.del$pH))
sul.cen = (white.wine.del$sulphates - mean(white.wine.del$sulphates))
alc.cen = (white.wine.del$alcohol - mean(white.wine.del$alcohol))
white.cen.mdl = lm(quality ~ fixed.cen + vol.cen + citric.cen + resid.cen + chlor.cen + free.cen + tot.cen + dens.cen + ph.cen + sul.cen + alc.cen, data=white.wine.del)
vif(white.cen.mdl)
```
<br />
Unfortunately, centering the variables did not reduce the VIFs for our model. Residual sugar, density, and alcohol still has a high VIF. 

Next, we will try deleting the density variable from our model, since it is the one with the largest VIF.
...
```{r}
white.den.del = lm(quality ~ .-density, data=white.wine.del)
vif(white.den.del)
```
<br />
By deleting the density variable, we successfully reduced all the VIFs to below 10.
